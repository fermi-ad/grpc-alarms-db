use dotenv::dotenv;
use std::{env, net::{Ipv4Addr, SocketAddr, SocketAddrV4}};
use tracing::info;
...
mod logging;
mod db;
use db::{postgres::PostgresDataStore, DataStore, DataRow};
mod services;
use services::alarm_lists::{AlarmListServiceImpl, proto, proto::alarm_list_service_server::AlarmListServiceServer};
...
async fn start_server<T: DataRow + 'static>(data_store: Box<dyn DataStore<T> + Send + Sync>) -> Result<(), Box<dyn std::error::Error>> {
